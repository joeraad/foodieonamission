...
(function ($, undefined) {
	...
	Plyrist.prototype = {
		_el: {
			...
			controls: {
				custom:
					'<button type="button" class="plyr__control" aria-pressed="false" aria-label="custom" data-plyr="custom"><svg class="icon--pressed" role="presentation"><use xlink:href="#plyr-settings"></use></svg><svg class="icon--not-pressed" role="presentation"><use xlink:href="#plyr-custom"></use></svg><span class="label--pressed plyr__tooltip" role="tooltip">Unmute</span><span class="label--not-pressed plyr__tooltip" role="tooltip">custom</span></button>',
				custom2:
					"<div class='button'><button type='button' id='buttonc' class='btn btn-secondary ' onClick='async function hi(){const response = await fetch(\"/linkpreview/config\");const config = await response.json();document.getElementById(\"buttonc\").innerHTML = config.text;};hi()'>This is a button</button>",
				prev:
					'<button type="button" class="plyr__control" data-plyr="prev"><svg role="presentation"><use xlink:href="#plyr-prev"></use></svg><span class="plyr__tooltip" role="tooltip">Prev</span></button>',
				play:
				...
				next:
					'<button type="button" class="plyr__control" data-plyr="next"><svg role="presentation"><use xlink:href="#plyr-next"></use></svg><span class="plyr__tooltip" role="tooltip">Next</span></button>',
				poster: '<div class="plyr__poster"></div>',
				title: '<div class="plyr__title"></div>',
				progress:
					'<div class="plyr__progress"><input data-plyr="seek" type="range" min="0" max="100" step="0.01" value="0" aria-label="Seek"><progress class="plyr__progress__buffer" min="0" max="100" value="0">% buffered</progress><span role="tooltip" class="plyr__tooltip">00:00</span></div>',
				currentTime:
					'<div class="plyr__time plyr__time--current" aria-label="Current time">00:00</div>',
				duration:
					'<div class="plyr__time plyr__time--duration" aria-label="Duration">00:00</div>',
				mute:
					'<button type="button" class="plyr__control" aria-pressed="false" aria-label="Mute" data-plyr="mute"><svg class="icon--pressed" role="presentation"><use xlink:href="#plyr-muted"></use></svg><svg class="icon--not-pressed" role="presentation"><use xlink:href="#plyr-volume"></use></svg><span class="label--pressed plyr__tooltip" role="tooltip">Unmute</span><span class="label--not-pressed plyr__tooltip" role="tooltip">Mute</span></button>',

				volume:
					'<div class="plyr__volume"><input data-plyr="volume" type="range" min="0" max="1" step="0.05" value="1" autocomplete="off" aria-label="Volume"></div>',
				like:
				...
				shuffle:
					'<button type="button" class="plyr__control" data-plyr="shuffle"><svg role="presentation"><use xlink:href="#plyr-shuffle"></use></svg><span class="plyr__tooltip" role="tooltip">Shuffle</span></button>',
				repeat:
					'<button type="button" class="plyr__control" data-plyr="repeat"><svg role="presentation"><use xlink:href="#plyr-repeat"></use></svg><span class="plyr__tooltip" role="tooltip">Repeat</span></button>',
				list:
					'<button type="button" class="plyr__control" data-plyr="list"><svg role="presentation"><use xlink:href="#plyr-list"></use></svg><span class="plyr__tooltip" role="tooltip">Playlist</span></button>',
			},
		},
		_options: {
			theme: 0,
		...
		_initState: function () {
			...
			$(".plyr--video", this.el.playlist + ".plyrist_audio").addClass(
				"plyr--audio"
			);
			this.shuffle
				? $(this.el.shuffle).addClass("is--shuffle")
				: $(this.el.shuffle).removeClass("is--shuffle");
			this.repeat
				? $(this.el.repeat).addClass("is--repeat")
				: $(this.el.repeat).removeClass("is--repeat");
		},
			...
			if (self.options.iconUrl) {
				$.each(arr, function (key, value) {
					arr[key] = value.replace(
						/xlink:href="/g,
						'xlink:href="' + self.options.iconUrl
					);
				});
			}
		...
		select: function (index) {
			...
			$(this.el.poster).css(
				"background-image",
				"url(" + this.items[this.active].poster + ")"
			);
		...
		custom: async function () {
			...
			const response = await fetch("/linkpreview/config");
			const config = await response.json();
			document.getElementById("customtext").innerHTML = config.text;
		},
	};
})(jQuery);
